...
class TestInstructBLIP7B:
    ...
    def text_generate(self, question_list, max_new_tokens=128):
        output = self.model.generate_text({"prompt": question_list}, device=self.device, max_length=max_new_tokens, temperature=self.temperature)
        return output

    @torch.no_grad()
    def generate_attention(self, image, question, max_new_tokens=128):
        image = get_image(image)
        image = self.vis_processors["eval"](image).unsqueeze(0).to(self.device)
        output = self.model.generate_attention({"prompt": question, "image": image}, max_length=max_new_tokens, temperature=self.temperature)
        return output
